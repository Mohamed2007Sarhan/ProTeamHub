...
<head>
  ...
  <title>container - Accounts | ProTeamHub</title>
            ...
            <ul class="navbar-nav flex-row align-items-center ms-auto">
              ...
              <li class="nav-item lh-1 me-3">
                <a
                  ...
                  aria-label="Star ProTeamHub/ProTeamHub-html-admin-template-free on GitHub">Star</a>
              </li>
                    ...
                    <a class="dropdown-item" href="#">
                      <div class="d-flex">
                        ...
                        <div class="flex-grow-1">
                          <span class="fw-semibold d-block"><?= htmlspecialchars($_SESSION['username']) ?> </span>
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            ...
            if (isset($_GET['delete_id'])) {
              ...
              if ($conx->query($sql) === TRUE) {
                echo "<script>alert('container deleted successfully');</script>";
              } else {
            ...
            if (isset($_POST['edit_id'])) {
              ...
              $sql = "UPDATE feedback SET username='$username', email='$email', message='$message', rebry='$rebry', user_id='$user_id', ip_address='$ip_address' WHERE id=$id";

              if ($conx->query($sql) === TRUE) {
                echo "<script>alert('container updated successfully');</script>";
              } else {
                echo "Error: " . $conx->error;
              }
            }

            // Fetch data from container table
            ...
            <body>
              ...
              <div class="container">
                    ...
                    <?php if ($result->num_rows > 0) : ?>
                      <?php while ($row = $result->fetch_assoc()) : ?>
                        <tr>
                          ...
                          <td>
                            <!-- Edit button -->
                            <button class="btn btn-edit" onclick="editcontainer(<?= $row['id'] ?>, '<?= $row['username'] ?>', '<?= $row['email'] ?>', '<?= $row['message'] ?>', '<?= $row['rebry'] ?>', '<?= $row['user_id'] ?>', '<?= $row['ip_address'] ?>')">Edit</button>
                            <!-- Delete button -->
                            <button class="btn btn-delete" onclick="window.location.href='?delete_id=<?= $row['id'] ?>'">Delete</button>
                          </td>
                        </tr>
                      <?php endwhile; ?>
                    <?php else : ?>
                      <tr>
                        <td colspan="8">No container available</td>
                      </tr>
                    <?php endif; ?>
                  </tbody>
                </table>
                ...
                <div id="editForm" class="edit-form">
                  ...
                  <form method="POST" action="">
                    ...
                    <label for="rebry">Rebry:</label>
                    <input type="text" id="rebry" name="rebry" required><br><br>
              ...
              <script>
                ...
                function editcontainer(id, username, email, message, rebry, user_id, ip_address) {
                  document.getElementById('edit_id').value = id;
                  document.getElementById('username').value = username;
                  document.getElementById('email').value = email;
                  document.getElementById('message').value = message;
                  document.getElementById('rebry').value = rebry;
