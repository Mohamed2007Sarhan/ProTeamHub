...
if (isset($_POST['submit'])) {
    ...
    $CodeReset = mysqli_real_escape_string($conx, encryptData(rand()));
    if (mysqli_num_rows(mysqli_query($conx, "SELECT * FROM register WHERE email='{$email}'")) > 0) {
        ...
        if ($query) {
            ...
            try {
                ...
                //Recipients
                $mail->setFrom('proteamhub44@gmail.com', 'Eagle');
                $mail->addAddress($email);
                //Content
                $mail->isHTML(true);                                  //Set email format to HTML
                $mail->Subject = 'Welecom To My Website';
                $mail->Body    = '<p> This is the Verifecation Link<b><a href="http://localhost/website/proteamhub/login/change-Password.php?Reset=' . $CodeReset . '">"http://localhost/website/proteamhub/login/change-Password.php?Reset=' . $CodeReset . '"</a></b></p>';
